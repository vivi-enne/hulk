...
def main():
    ...
    with open("2024-07-18-HULKs-vs-NomadZ/first-half/10.1.24.32/2024-07-18_10:48:50/outputs.mcap", "rb") as file:
        ...
        for i, channel in enumerate(reader.get_summary().channels.values()):
            ...
            if topic == "Control.main_outputs.role":  # or topic == "Control.main_outputs.sensor_data" or topic == "Control.main_outputs.motor_commands":
                print(f"{topic = }")

                number_messages = reader.get_summary().statistics.channel_message_counts[channel.id]
                ...
                if j == 0:
                    ...
                    row = pl.from_dicts(iterate_messages(reader, [topic]), infer_schema_length=number_messages, strict=False)
                    print(f"{row.columns = }")
                    ...
                    if len(df) != len(row):
                        raise ValueError("Both DataFrames must have the same number of rows to align.")

                    # Align columns: Ensure both DataFrames have the same columns
                    all_columns = set(df.columns).union(row.columns)

                    # Add missing columns to both DataFrames
                    df = df.with_columns([pl.lit(None, dtype=row.schema[col] if col in row.schema else pl.Object).alias(col) for col in all_columns if col not in df.columns])
                    row = row.with_columns([pl.lit(None, dtype=df.schema[col] if col in df.schema else pl.Object).alias(col) for col in all_columns if col not in row.columns])

                    # Merge the row data into the original DataFrame on a row-by-row basis
                    df = df.with_columns([pl.when(row[col].is_null()).then(df[col]).otherwise(row[col]).alias(col) for col in row.columns])
...
def iterate_messages(reader, topics: list[str]):
    ...
    for i, (_, channel, message) in enumerate(reader.iter_messages(topics=topics)):
        ...
        if type(message_data) is dict:
            ...
            new_data.update({f"{topic}.{key}": value for key, value in message_data.items()})
