...
use crate::{
    ...
    SubscriptionStatusSnapshot, dynamic_payload_to_json, history::TimeIndexedHistory,
};
...
pub struct SubscriptionUpdateReceiver {
    inner: broadcast::Receiver<SubscriptionUpdate>,
}

impl SubscriptionUpdateReceiver {
    fn new(inner: broadcast::Receiver<SubscriptionUpdate>) -> Self {
        Self { inner }
    }
    ...
    pub fn try_recv(&mut self) -> Result<Option<SubscriptionUpdate>, SubscriptionUpdateClosed> {
        match self.inner.try_recv() {
            ...
            Err(broadcast::error::TryRecvError::Closed) => Err(SubscriptionUpdateClosed),
        }
    }
}
...
impl<V> SubscriptionState<V> {
    pub(crate) fn new(status: SubscriptionStatusSnapshot, retention: RetentionPolicy) -> Self {
        ...
        Self {
            ...
            history,
            meta: Mutex::new(SubscriptionMeta::Open {
                status,
                updates,
                receive_task: None,
            }),
        ...
        V: Send + Sync + 'static,
    {
        ...
        let receive_task = tokio::spawn(receive_loop(weak_state, cancellation));
    ...
    pub(crate) fn set_receive_task(&self, abort_handle: AbortHandle) {
        ...
        match &mut *meta {
            SubscriptionMeta::Open { receive_task, .. } => {
                if let Some(previous) = receive_task.replace(abort_handle) {
                    previous.abort();
                }
            }
            SubscriptionMeta::Closed { .. } => abort_handle.abort(),
        }
    }

    pub(crate) fn close(&self) {
        let mut meta = self.meta.lock();
        let (closed_status, receive_task, updates) = match &mut *meta {
            SubscriptionMeta::Open {
                status,
                updates,
                receive_task,
            } => {
                status.set_status(SubscriptionStatus::Closed);
                (status.clone(), receive_task.take(), updates.clone())
            }
            SubscriptionMeta::Closed { .. } => return,
        };
        *meta = SubscriptionMeta::Closed {
    ...
    fn publisher_count(&self) -> usize {
        ...
        let status = match &*meta {
            SubscriptionMeta::Open { status, .. } | SubscriptionMeta::Closed { status } => status,
        };
        let Some(topic) = status.resolved_topic() else {
            return 0;
        };
...
impl<V> Drop for SubscriptionState<V> {
    fn drop(&mut self) {
        self.cancellation_token.cancel();
        let mut meta = self.meta.lock();
        if let SubscriptionMeta::Open { receive_task, .. } = &mut *meta {
            if let Some(receive_task) = receive_task.take() {
                receive_task.abort();
            }
        }
    }
